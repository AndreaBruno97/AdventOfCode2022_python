...
class Part_2(BaseClass):
    ...
    def execute_internal(self, filepath):
        ...
        root_node = Node("/", Node.TYPE_DIR, 0, None)
        node_list = [root_node]
        ...
        used_space = directory_size(node_list, root_node)
        unused_space = TOTAL_SPACE - used_space
        space_to_free = UPDATE_SIZE - unused_space

        directory_to_delete_size = used_space
        ...
        for cur_directory in [x for x in node_list if x.node_type == Node.TYPE_DIR]:
            ...
            if space_to_free < cur_directory_size < directory_to_delete_size:
                directory_to_delete_size = cur_directory_size

        return directory_to_delete_size
